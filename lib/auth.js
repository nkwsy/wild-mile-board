...
/* BOARD_SECRET signs tokens. Without one, derive a stable key from the password
   ...
   everyone out, which is the behaviour you want anyway). With neither set there
   is no key worth the name — configured() makes sure we never verify against it. */
function secret() {
  return process.env.BOARD_SECRET ? Buffer.from(process.env.BOARD_SECRET) : sha256("sig:" + password());
}

/* A deployment missing both env vars would otherwise sign with a constant
   anyone reading this file could derive, so every cookie would verify. */
const configured = () => !!(process.env.BOARD_SECRET || password());

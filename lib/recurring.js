...
const { log, newId, bad } = require("./issues");
...
function duesFor(t, from, to) {
  ...
  for (const y of years) {
    ...
    const to0 = parseDay(`${y}-${t.seasonEnd || "12-31"}`);
    if (isNaN(from0) || isNaN(to0) || to0 < from0) continue;
...
async function generate({ templateId, from, to, days, actor } = {}) {
  await init();
  const start = /^\d{4}-\d{2}-\d{2}$/.test(str(from)) ? str(from) : ymd(new Date());
  const horizon = Math.min(Math.max(Number(days) || 60, 1), 400);
  const end = /^\d{4}-\d{2}-\d{2}$/.test(str(to))
    ? str(to)
    : ymd(new Date(parseDay(start).getTime() + horizon * DAY));

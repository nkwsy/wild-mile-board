...
/* Create the table on first use, and fill an empty one with the seed cards. */
function init() {
  ...
  ready = (async () => {
    ...
    const { rows } = await run("SELECT count(*)::int AS n FROM cards");
...
/* Accepts whatever the client sent and returns a card safe to store. */
function clean(input, base) {
  const prev = base || {};
  const pick = (key, fallback) => (input[key] === undefined ? fallback : input[key]);
  const ord = Number(pick("ord", prev.ord));
  return {
    id: str(prev.id || input.id) || "c-" + Date.now().toString(36),
    title: str(pick("title", prev.title)).slice(0, 140) || "Untitled issue",
    desc: str(pick("desc", prev.desc)).slice(0, 1200),
    reporter: str(pick("reporter", prev.reporter)).slice(0, 80),
    loc: str(pick("loc", prev.loc)).slice(0, 80),
    prio: PRIOS.includes(pick("prio", prev.prio)) ? pick("prio", prev.prio) : "Medium",
    date: str(pick("date", prev.date)).slice(0, 10),
    link: str(pick("link", prev.link)).slice(0, 500),
    col: COLS.includes(pick("col", prev.col)) ? pick("col", prev.col) : "new",
...
const RETURNING = "RETURNING id, title, descr, reporter, loc, prio, reported, link, col, ord, updated_at";

/* Insert or overwrite one whole card. Last write wins, which is what we want. */
async function write(card) {
  const c = clean(card, null);
...
async function patch(id, fields) {
  await init();
  const { rows } = await run("SELECT * FROM cards WHERE id = $1", [String(id)]);
  if (!rows.length) return null;
  return write({ ...clean(fields, rowToCard(rows[0])), id: rows[0].id });
}

async function list() {
  await init();
  const { rows } = await run(
    "SELECT id, title, descr, reporter, loc, prio, reported, link, col, ord, updated_at" +
...
module.exports = { list, upsert, patch, remove, close, COLS, PRIOS };

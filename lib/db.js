...
/* Match a free-text place against the seeded locations, by name or alias, so a
   card that said "Nat Geo island" lands on the Nat Geo record. Returns an id or
   null; the text is kept either way, because the text is what the reporter said. */
async function resolveLocation(text) {
  const t = String(text || "").trim().toLowerCase();
  if (!t) return null;
  const { rows } = await run(
    `SELECT id FROM locations
      WHERE lower(name) = $1 OR $1 = ANY (SELECT lower(a) FROM unnest(aliases) a)
      ORDER BY sort LIMIT 1`, [t]);
  if (rows.length) return rows[0].id;
  const { rows: loose } = await run(
    `SELECT id FROM locations
      WHERE position(lower(name) in $1::text) > 0
         OR EXISTS (SELECT 1 FROM unnest(aliases) a WHERE position(lower(a) in $1::text) > 0)
      ORDER BY length(name) DESC LIMIT 1`, [t]);

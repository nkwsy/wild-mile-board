...
function getPool() {
  ...
  const local = /localhost|127\.0\.0\.1|^postgres(ql)?:\/\/\/|host=\//.test(url);
  pool = new Pool({
...
async function carryOverCards() {
  ...
  for (const c of rows) {
    ...
    const reported = isDay(c.reported) ? c.reported : null;
...
async function seedIssues() {
  for (const s of ISSUES) {
    const reported = isDay(s.reported_on) ? s.reported_on : null;
    // rowCount, not a blind insert: two cold functions can reach a brand new
    // database at the same time, and the second one must not add a second
    // "Reported" line to a history the first one already wrote.
    const { rowCount } = await run(
      `INSERT INTO issues (id, title, descr, severity, status, location_id, location_text,

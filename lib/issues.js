...
const { str, trim, bad, newId, slug, iso, dayOrNull } = require("./util");
...
function where(f = {}) {
  const parts = [];
  const params = [];
  /* Adds one value and hands back the placeholder that stands for it. */
  ...
  if (statuses.length) parts.push(`i.status = ANY(${p(statuses)})`);

  const sevs = list(f.severity).filter(s => SEVERITIES.includes(s));
  if (sevs.length) parts.push(`i.severity = ANY(${p(sevs)})`);

  if (str(f.location).trim()) {
    const loc = p(str(f.location).trim());
    parts.push(`(i.location_id = ${loc} OR lower(i.location_text) = lower(${loc}))`);
  }
  if (str(f.assignee).trim()) parts.push(`lower(i.assignee) = lower(${p(str(f.assignee).trim())})`);
  if (str(f.template).trim()) parts.push(`i.template_id = ${p(str(f.template).trim())}`);

  if (str(f.q).trim()) {
    const like = p("%" + str(f.q).trim().toLowerCase() + "%");
    parts.push(`(lower(i.title) LIKE ${like} OR lower(i.descr) LIKE ${like}
                 OR lower(i.location_text) LIKE ${like} OR lower(i.reporter) LIKE ${like}
                 OR lower(i.assignee) LIKE ${like})`);
  }
  if (f.staleDays) {
    parts.push(`i.status <> 'done'
                AND i.touched_at < now() - make_interval(days => ${p(Number(f.staleDays))}::int)`);
  }
...
async function cleanInput(input, existing) {
  ...
  if (has("reportedOn")) out.reported_on = dayOrNull(input.reportedOn);
  if (has("dueOn")) out.due_on = dayOrNull(input.dueOn);
...
async function updateIssue(id, input, actor) {
  ...
  await tx(async q => {
    ...
    // Not when closing: fields.status has been removed and closeIssues() below
    // writes the row that says what happened.
    if (statusChange && !closing) {
...
async function rememberPerson(name, role) {
  ...
  await run(
    ...
    ["ppl-" + slug(clean), clean, role || "volunteer"]);
}

module.exports = {
  listIssues, getIssue, createIssue, updateIssue, removeIssue,
  assign, respond, closeIssues, reopen, addNote,
  locations, people, rememberPerson, filtersFromQuery,
  log                     // photos and recurring write history rows of their own
};
